...
const tileStatus = (tile: Tile, lessonsCompleted: number): TileStatus => {
  ...
  if (tileIndex < 0) return "LOCKED"; // Handle case where tile is not found
...
const TileIcon = ({
  ...
  switch (tileType) {
    ...
    default:
      return <LockSvg />; // Fallback for unknown tile types
  }
};
...
const getTileColors = ({
  ...
  switch (status) {
    case "LOCKED":
      return tileType === "fast-forward" ? defaultColors : "border-[#b7b7b7] bg-[#e5e5e5]";
    case "COMPLETE":
      return "border-yellow-500 bg-yellow-400";
    case "ACTIVE":
      return defaultColors;
    default:
      return defaultColors; // Fallback for unknown status
  }
};
...
const IslandSection = ({ island }: { island: Unit }) => { // Added type for island
                      ...
                      <div
                        className={[
                          ...
                          getTileLeftClassName({
                            index: i,
                            unitNumber: island.unitNumber,
                            tilesLength: island.tiles.length,
                          }),
                        ].join(" ")}
                      >
                        {tile.type === "fast-forward" && status === "LOCKED" ? (
                          <HoverLabel
                            text="Jump here?"
                            textColor={island.textColor}
                          />
                        ) : selectedTile !== i && status === "ACTIVE" ? (
                          <HoverLabel text="Start" textColor={island.textColor} />
                        ) : null}
                        <LessonCompletionSvg
                        ...
                        <button
                          className={[
                            ...
                            getTileColors({
                              tileType: tile.type,
                              status,
                              defaultColors: `${island.borderColor} ${island.backgroundColor}`,
                            }),
                          ].join(" ")}
                          onClick={() => {
                            if (
                              ...
                              void router.push(
                                `/lesson?fast-forward=${island.unitNumber}`,
                              );
                              return;
                            }
                            setSelectedTile(i);
                  ...
                  switch (tile.type) {
                    ...
                    case "fast-forward":
                      return status === "LOCKED"
                        ...
                        : tile.description;
                    case "trophy":
                      return `Unit ${island.unitNumber} review`;
                    case "treasure":
                      return "";
                    default:
                      return ""; // Fallback for unknown tile types
                  }
                })()}
                status={status}
  ...
  return (
    ...
    >
      <header className="flex items-center justify-between gap-4 p-4">
        ...
        <Link
          href={`https://duolingo.com/guidebook/${language?.code || 'en'}/${unitNumber}`}
          className={[
